...
st.set_page_config(page_title = "Zinga Rene", page_icon = ":100:", layout = "wide")

# Cria a barra lateral do aplicativo com opções e instruções
with st.sidebar:
    st.header("Configurações")
    api_key = st.text_input("Enter here your GROQ API Key e pressione Enter", type = "password")
    st.divider()
    st.subheader("Instructions")
    st.write("1) Enter your key in the field above..\n2) Type your question or doubt.\n3) Click to Send.")
    st.info("Warning: The IA can make mistakes. Verify the critics Facts.")
    st.link_button("Click Here If You Need Support", "https://www.datascienceacademy.com.br/suportedsa")
    st.link_button("Click Here To Learn About RAG", "https://www.datascienceacademy.com.br/todoscursosdsa")

# Exibe títulos e informações sobre o projeto
st.title("Zinga Rene")
st.title("Projeto")
st.title(" Document Assistent (Using RAG)")
...
if not api_key:
    st.warning("Enter the GROQ API Key in the sidebar to continue.")
    st.stop()
...
pdf_file = st.file_uploader("Please send a PDF from the legal area (contract, opinion, decision, consolidated law…)", type = ["pdf"])
...
system_block = """Você é um assistente de consulta de documentos com formato .pdf que responde usando estritamente o conteúdo do PDF fornecido quando possível.
...
pergunta = st.text_area("Write your legal question.", height = 120, placeholder = "Ex.: What is the most important skill in the age of Artificial Intelligence?")

# Cria duas colunas e insere o botão na primeira
col1, col2 = st.columns([1,1])
with col1:
    # Botão de envio da pergunta
    btn = st.button("To ask")
...
if btn:
    ...
    if not retriever:
        st.error("Send a PDF first to enable RAG.")
        st.stop()
